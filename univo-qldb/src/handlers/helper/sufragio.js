...
async function addSufragioId(txn, docId, dui) {
  const statement =
    "UPDATE Sufragios as b SET b.sufragioId = ? WHERE b.dui = ?";
...
const crearSufragio = async (
  ...
  logger.debug(
    `Funcion crearSufragio: Nombre: ${codigo}, DUI: ${dui}, Departamento: ${departamento}, Municipio: ${municipio} y Sexo: ${sexo}`
  );
  ...
  await qldbDriver.executeLambda(async (txn) => {
    ...
    if (recordsReturned === 0) {
      const sufragioDoc = {
        ...
        estado: 0,
        uuid,
        eventname: "ingresoCentroVotacion",
        status: 0,
        votoId: 0,
        events: event,
      };
      ...
      throw new sufragioError(
        400,
        "Error de Integridad de Sufragio",
        `Ya existe un registro con el ${dui}. No agregaran mas registros`
      );
    }
  });
  return sufragio;
...
async function updateSufragio(
  ...
  console.log(
    `En la actualizacion de estado de sufragioId ${sufragioId} y evento ${eventInfo}`
  );
  const statement =
    "UPDATE Sufragios SET estado = ?, eventname = ?, status = ?, votoId = ?, events = ? WHERE sufragioId = ?";
  return txn.execute(
    statement,
    newEstado,
    eventname,
...
const verificarSufragio = async (sufragioId, eventInfo) => {
  console.log(
    `En la funcion de ejecucion de sufragio ${sufragioId}, y eventInfo ${eventInfo}`
  );
  ...
  await qldbDriver.executeLambda(async (txn) => {
    ...
    if (resultList.length === 0) {
      throw new sufragioError(
        400,
        "Error de integridad de sufragio",
        `Registro de sufragio con el sufragioId ${sufragioId} no existe`
      );
    } else {
      const newEstado = 1;
      const newStatus = 1;
...
const ejecutarSufragio = async (sufragioId, eventInfo, votoId) => {
  console.log(
    `En la funcion de ejecutar el sufragio ${sufragioId}, y eventInfo ${eventInfo}`
  );
